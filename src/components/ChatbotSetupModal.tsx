...
export default function ChatbotSetupModal({ bot, onClose }: Props): JSX.Element {
  ...
  const snippet = `// After your bot gets a response from OpenAI, report usage to
// Community Analytics. Fire-and-forget so it never blocks the user.
// 'response' is the OpenAI Responses API result.
await fetch("${endpoint}", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    chatbot_id: "${bot.id}",
    openai_response_id: response.id,           // enables dedup of retries
    openai_prompt_id: response.prompt?.id,     // optional
    conversation_id: conversationId,           // your thread/session id
    input_tokens: response.usage.input_tokens,
    output_tokens: response.usage.output_tokens,
    total_tokens: response.usage.total_tokens,
    model: response.model,                     // e.g. "${bot.model}"
    message_count: 1,
    input_text: userQuery,                     // optional — only stored if capture is on
      ...
      >
        ...
        <div className="mt-5 flex flex-col gap-4">
          ...
          <div className="rounded-md bg-surface-alt px-3.5 py-3 text-11.5 leading-relaxed text-text-secondary">
            ...
            <code className="font-mono">input_text</code> — only normalized questions are stored,
