...
- Raw API keys are **never persisted** — only `keyLast4` + a masked display
  ...
  wired in.
- This local store backs the **demo account only**; real accounts use Supabase
  (`src/lib/supabaseBackend.ts`). `src/lib/session.tsx` routes between them and
  the pages talk only to its exported API, so neither backend leaks into them.

## Project layout

```
api/                      Vercel serverless functions (used by real accounts)
  _lib/                   supabase admin client, auth, zod schemas
  webhook/usage.ts        POST — ingest usage events (OpenAI Responses contract)
  analytics/dashboard.ts  GET  — aggregate usage into a TenantDataset
  analytics/cost.ts       GET  — total + per-model cost for a client over a period
  auth/bootstrap.ts       POST — ensure tenant + app_metadata.tenant_id claim
  chatbots/               GET/POST list+create, PATCH/DELETE per id
  tenant/index.ts         PATCH — workspace name / budget / alert threshold
src/
  components/
    layout/               Sidebar (tenant + logout + admin nav), TopBar (bot selector)
    charts/               5 Recharts charts
    ui/                    StatCard / TrendBadge / Toggle / Card / EmptyState / Logo
    ConnectChatbotModal.tsx   connect/edit a chatbot
    ChatbotSetupModal.tsx     per-bot ingest endpoint + copy-paste snippet
  pages/                  Login, Dashboard, Chatbots, Usage, Reports, Settings, CostExplorer
  lib/
    store.ts              localStorage store — demo account (seeds NY Yankees)
    supabaseBackend.ts    Supabase auth + tenant/chatbot/analytics for real accounts
    session.tsx           SessionProvider (routes demo/Supabase) + analytics hooks
    dataset.ts            Tenant → TenantDataset (verbatim Yankees / generated others)
    mockData.ts           buildAnalytics(dataset, chatbot, period) → all page data
    utils.ts              formatters + deterministic seeded series
    supabase.ts, config.ts, appState.tsx, cx.ts
  types/                  shared domain types
supabase/schema.sql       tables + indexes + RLS + functions
```
...
| `ANALYTICS_API_TOKEN` | serverless | privileged READ token for `GET /api/analytics/cost` (server-to-server: backend service / email / Claude job) |
...
- **Admins** (`app_metadata.role = 'admin'`) additionally see the **Cost
  Explorer** nav item and can query any client's cost. Grant it from the
  Supabase dashboard (Authentication → Users → a user → edit `app_metadata` →
  add `"role": "admin"`) or via the Admin API; the user must sign out / in to
  pick up the refreshed claim.
...
| `GET /api/analytics/cost` | total + per-model cost for a `client_id` over `from`/`to` (or `period`) |
| `POST /api/webhook/usage` | ingest one reply's usage (Zod-validated, secret-checked, deduplicated) |

**Write contract (`POST /api/webhook/usage`).** Accepts the OpenAI **Responses
API** shape — `input_tokens` / `output_tokens` / `total_tokens`, `model`, and
optional `openai_response_id`, `openai_prompt_id`, `input_text` — and also the
Chat Completions names (`prompt_tokens` / `completion_tokens`) as aliases. Cost
is always computed server-side from the model (the bot reports usage, never
cost). When `openai_response_id` is present the event is **idempotent** —
retries / `previous_response_id` resends are accepted once and never
double-counted. Raw `input_text` is stored **only** when the bot has
`capture_messages = true` (privacy-safe default off); otherwise it is used just
to increment the normalized top-questions counter.

**Read contract (`GET /api/analytics/cost`).** Query by `client_id` (tenant
UUID) + `from`/`to` (ISO) or `period` (`7d|30d|90d|all`). Two auth modes:
`Authorization: Bearer <ANALYTICS_API_TOKEN>` for a privileged server-to-server
caller (any client), or a user JWT (own client only; `app_metadata.role =
'admin'` unlocks any client). Aggregation runs in SQL
(`analytics_cost_summary`) so totals stay correct at any row volume. The
dashboard's admin-only **Cost Explorer** page is a thin UI over this endpoint.
...
caller's tenant via the JWT's **`app_metadata`**. It also adds the OpenAI id /
`input_text` columns, the dedup unique index, and the `analytics_cost_summary`
function (all idempotent — safe to re-run).
...
The usage webhook accepts an optional `input_text` field (or `user_message`).
When present **and** the target bot has `capture_messages = true`, the handler
normalizes the text (collapse whitespace, strip quotes, cap length) and calls the
atomic `bump_top_question()` function, which upserts into `top_questions` and
increments a running count. **Raw transcripts are never stored** unless capture
is on — otherwise only a normalized question plus its count is kept, so the "Most
Frequent Messages" panel works without retaining sensitive message bodies. The
tenant's top questions are returned inline by `GET /api/analytics/dashboard`.
Capture is opt-in
