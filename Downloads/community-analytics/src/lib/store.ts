...
import { YANKEES_ALL_METRICS, YANKEES_TEAM } from './dataset';

// ---------------------------------------------------------------------------
// Local persistence layer.
//
// This is the app's data + auth store while the Supabase backend is not yet
// wired in. Accounts, tenants and chatbot connections are persisted in
// localStorage; passwords are stored only as a salted SHA-256 hash (never in
// plain text), and raw API keys are never persisted — only a masked view.
//
// When Supabase is connected, this module is the single seam to replace:
// swap these functions for Supabase Auth + Postgres calls and the rest of the
// app (which talks only to the exported API below) keeps working unchanged.
// ---------------------------------------------------------------------------
...
// Touch YANKEES_ALL_METRICS import so tree-shaking keeps the dataset module's
// verbatim values referenced from one place (documentation of intent).
void YANKEES_ALL_METRICS;
...
export async function register(input: RegisterInput): Promise<AuthResult> {
  ...
  if (login.length < 3) return { ok: false, error: 'Username must be at least 3 characters.' };
  if (input.password.length < 6) {
    return { ok: false, error: 'Password must be at least 6 characters.' };
  }
  if (workspaceName.length < 2) return { ok: false, error: 'Enter a workspace name.' };

  const store = loadStore();
  if (store.accounts.some((a) => a.login.toLowerCase() === login.toLowerCase())) {
    return { ok: false, error: 'That username is already taken.' };
  }

  const ownerName = input.ownerName?.trim() || workspaceName + ' Owner';
  const ownerEmail =
    input.ownerEmail?.trim() ||
    login.toLowerCase().replace(/[^a-z0-9]/g, '') + '@' + slugify(workspaceName) + '.com';
  const team: TeamMember[] = [
    { ini: initials(ownerName), name: ownerName, email: ownerEmail, role: 'Owner' },
  ];

  const tenant: Tenant = {
    id: genId(),
    workspace: { name: workspaceName, timezone: 'America/New_York (ET)', currency: 'USD ($)' },
    billing: {
      plan: 'Starter',
      planPrice: '$0/mo',
      nextInvoice: '—',
