...
// Idempotent. Ensures the signed-in user has a tenant:
//   - if app_metadata.tenant_id is already set, returns that tenant
//   - otherwise creates a tenant, a default message rate, and stamps
//     app_metadata.tenant_id on the user (requires service role)
// After a fresh create the client must refreshSession() so the new JWT carries
// the tenant_id claim that the other endpoints read.
